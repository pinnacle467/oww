...
export function ToursDropdown({ solid, currentPath, parentLabel, parentTo }) {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api/journeys`).then(({ data }) => {
      // Filter to active type=tour items with usable slugs. type may be
      // missing on legacy rows so treat undefined as "tour".
      const usable = (data || []).filter(
        (j) => (j.type === "tour" || !j.type) && j.slug,
      );
      setTours(usable);
    }).catch(() => setTours([]));
  }, []);
