...
export function HeroSlideshow() {
  ...
  const brandName = useText("brand.name", BRAND.name);
  const brandSub = useText("brand.sub", BRAND.sub);
  const tagline = useRichText("brand.tagline", BRAND.tagline);
  ...
  useEffect(() => {
    ...
    const dwell = reduceMotion ? 9000 : 5500;
    ...
    <section className="relative h-[100svh] w-full overflow-hidden bg-ink" data-testid="hero-slideshow">
      ...
      <div className="absolute inset-x-0 bottom-0 z-10">
        <div className="mx-auto max-w-7xl px-5 sm:px-8 pb-12 sm:pb-16">
          <div
            // glass-dark gives a deep ink-tinted backdrop with blur so the gold
            // eyebrow + white headline stay legible on ANY hero photo — sage,
            // sunlit ridge, dusk lake, whatever the client uploads next.
          ...
          >
            <p className="label-eyebrow text-gold mb-4 drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]">{brandName} {brandSub}</p>
